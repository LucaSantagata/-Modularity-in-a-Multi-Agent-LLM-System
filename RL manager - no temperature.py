...
# RL Parameters
RL_LEARNING_RATE = 0.6  # Increased to 0.6 for faster adaptation
# Temperature removed (Implicitly 1.0)
...
CHECKPOINT_WEIGHTS = os.path.join(BASE_DIR, "checkpoint_weights_log.pkl")
...
class RLManager:
    def __init__(self, num_arms, num_agents, lr):
    ...
    def assign_arms(self):
        ...
        # 1. Softmax Assignment (Using raw weights, Implicit T=1.0)
        # This will result in much sharper probabilities than T=2.0
...
if __name__ == "__main__":
    ...
    # 2. Setup with RL (Temperature Removed)
    agents = [Agent(i, N_AGENTS, N_ARMS, N_ROUNDS) for i in range(N_AGENTS)]
    manager = RLManager(N_ARMS, N_AGENTS, learning_rate=RL_LEARNING_RATE)
    ...
    if start_round > 0:
        ...
        # Restore Manager Weights
        if weights_log:
            print("Restoring Manager Weights...")
            manager.weights = weights_log[-1].copy()
            
    print(f"Starting RL Simulation. Alpha={RL_LEARNING_RATE}, Temp=Fixed(1.0)")
    ...
    experiment_df.to_csv("final_rl_results_low_churn.csv")
    pd.DataFrame(message_log).to_csv("final_rl_messages_low_churn.csv")
    
    with open("final_weights_history_low_churn.pkl", "wb") as f:
